...
class ColorUtils {
  static String colorToHex(Color color) {
    return ColorUtils.rgb(color.red.toDouble(), color.green.toDouble(), color.blue.toDouble());
  }
  ...
  static String rgb(double r, double g, double b) =>
      '#${ColorUtils.decToHex(r)}${ColorUtils.decToHex(g)}${ColorUtils.decToHex(b)}';
  ...
  static String hsl(double h, double s, double l) {
    if (s == 0.0) {
      final String partialHex = ColorUtils.decToHex(255 * l);
      return '#$partialHex$partialHex$partialHex';
    } else {
      final double m2 = l <= 0.5 ? l * (s + 1) : l + s - (l * s);
      final double m1 = (l * 2) - m2;
      return '#${ColorUtils.hueToRgb(m1, m2, h * 6 + 2)}${ColorUtils.hueToRgb(m1, m2, h * 6)}${ColorUtils.hueToRgb(m1, m2, h * 6 - 2)}';
    }
  }

  static String correctedHsl(double h, double s, double l) {
    final List<double> correctors = <double>[0.55, 0.5, 0.5, 0.46, 0.6, 0.55, 0.55];
    final double corrector = correctors[(h * 6 + 0.5).floor()];
    return ColorUtils.hsl(
        h,
        s,
        l < 0.5
            ? l * corrector * 2
